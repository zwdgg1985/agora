...
extern(D):
    ...
    ***************************************************************************/
        ...
        SCPQuorumSet quorum_set)
    {
        ...
        auto local_quorum_set = cast()this.scp.getLocalQuorumSet();
    ...
    ***************************************************************************/
    ...
    {
        logCall("{} {}", this.key_pair.address.prettify);
    ...
    ***************************************************************************/
    ...
    {
        // todo: verify the envelope signature before passing it on to SCP
        logCall("{} {}", this.key_pair.address.prettify);
    ...
    {
        // todo: sign the envelope (schnorr)
        logCall("{} {}", this.key_pair.address.prettify);
    }
        ...
        ref const(Value) value)
    {
        ...
        if (!this.ledger.txSetExternalized(tx_set))
            assert(0);
    }
    ...
    public override void setupTimer (ulong slotIndex, int timerID,
        milliseconds timeout, CPPDelegate!SCPCallback* cb)
    {
        scope (failure) assert(0);
        logCall("{} {}", this.key_pair.address.prettify);

        if (cb is null || timeout == 0)
        {
            this.timers.remove(timerID);
            return;
        }

        this.timers.put(timerID);
...
/// workaround for logger not being nothrow
private void logCall (T...)(lazy T args, string func = __FUNCTION__) nothrow
{
    try
    {
        log.trace(args[0], func, args[1 .. $]);
    }
    catch (Exception)
    {
        // don't care
