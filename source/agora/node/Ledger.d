...
public class Ledger
{
    ...
    ***************************************************************************/
    ...
    private void tryMakingBlock () @safe
    {
        ...
        {
            if (tx.isValid(&this.utxo_set.findUTXO))
            {
                hashes ~= hash;
                txs ~= tx;
            }
    ...
    ***************************************************************************/
    ...
    {
        this.utxo_set.reset();
        return tx.isValid(&this.utxo_set.findUTXO);
    }
    ...
    ***************************************************************************/
    ...
    public bool isValidBlock (const ref Block block) nothrow @safe
    {
        ...
        this.utxo_set.reset();
        return block.isValid(last_block.header.height,
            last_block.header.hashFull, &this.utxo_set.findUTXO);
    }
...
unittest
{
    ...
    auto utxo_set = new UTXOSet(1024, ":memory:");
